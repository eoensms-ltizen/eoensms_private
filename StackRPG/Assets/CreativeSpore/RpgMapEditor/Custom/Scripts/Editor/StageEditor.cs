...
{
    public virtual StageData m_stageData { get { return (StageData)target; } }
    ...
    void OnEnable()
    {
        ...
        m_layerList = new ReorderableList(serializedObject, serializedObject.FindProperty("m_stage").FindPropertyRelative("m_makeUnitPositions"), true, true, true, true);
        ...
        m_layerList.onAddCallback = (ReorderableList list) =>
        {
            ...
            m_stageData.m_stage.m_makeUnitPositions.Add(new StartingPoint());
        };
    }
    ...
    void DrawEditTab()
    {
        EditorGUILayout.LabelField("Player Count : " + m_stageData.m_stage.m_makeUnitPositions.Count);
    ...
    public void OnSceneGUI()
    {
        ...
        //! StartPoint
        for (int i = 0; i < m_stageData.m_stage.m_makeUnitPositions[m_userIdx].m_positions.Count; ++i)
        {
            Vector2 position = m_stageData.m_stage.m_makeUnitPositions[m_userIdx].m_positions[i];
            DrawTileWithOutline((int)position.x, (int)position.y, new Color(0f, 1f, 0f, 0.2f), new Color(0f, 1f, 0f, 1f));
        }

        //! AttackPoint
        DrawTileWithOutline((int)m_stageData.m_stage.m_attackPoint.x, (int)m_stageData.m_stage.m_attackPoint.y, new Color(0f, 1f, 0f, 0.2f), new Color(0f, 1f, 0f, 1f));
    }
