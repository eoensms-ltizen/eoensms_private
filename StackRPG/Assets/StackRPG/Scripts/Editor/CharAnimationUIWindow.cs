...
public class CharAnimationUIWindow : EditorWindow
{
    ...
    CharAnimationControllerUI m_animCtrl;
    ...
    public static void Init(CharAnimationControllerUI animCtrl)
    {
        // Get existing open window or if none, make a new one:
        CharAnimationUIWindow window = (CharAnimationUIWindow)EditorWindow.GetWindow(typeof(CharAnimationUIWindow), false, "Char. Animation");
    ...
    void OnGUI()
    {
        ...
        {
            CharAnimationControllerUI animCtrl = Selection.activeGameObject != null ? Selection.activeGameObject.GetComponent<CharAnimationControllerUI>() : null;
            if (animCtrl != null)
            {
                m_animCtrl = animCtrl;
                if (animCtrl.CreateSpriteFrames())
    ...
    void DrawGUI()
    {
        ...
        m_animCtrl.CharsetType = (CharAnimationControllerUI.eCharSetType)EditorGUILayout.EnumPopup("Charset Type", m_animCtrl.CharsetType);
        ...
        {
            m_animCtrl.CurrentDir = (CharAnimationControllerUI.eDir)EditorGUILayout.EnumPopup("View Direction", m_animCtrl.CurrentDir);
            EditorGUI.BeginChangeCheck();
        ...
        foreach (CharAnimationControllerUI.eDir dir in System.Enum.GetValues(typeof(CharAnimationControllerUI.eDir)))
        {
            ...
            else if (m_viewMode == eCharAnimEditMode.MoveInCircles)
            {
                switch (dir)
                {
                    case CharAnimationControllerUI.eDir.DOWN: r2 = new Rect(r.x, r.y + fWalkProg * fWalkDistY, sprFrame.rect.width, sprFrame.rect.height); break;
                    case CharAnimationControllerUI.eDir.LEFT: r2 = new Rect(r.x + (1 - fWalkProg) * fWalkDistX, r.y, sprFrame.rect.width, sprFrame.rect.height); break;
                    case CharAnimationControllerUI.eDir.RIGHT: r2 = new Rect(r.x + fWalkProg * fWalkDistX, r.y + fWalkDistY, sprFrame.rect.width, sprFrame.rect.height); break;
                    case CharAnimationControllerUI.eDir.UP: r2 = new Rect(r.x + fWalkDistX, r.y + (1f - fWalkProg) * fWalkDistY, sprFrame.rect.width, sprFrame.rect.height); break;
                }
            }
