...
public struct Damage
{
    ...
    public Damage(AttackType attackType, int power, Vector2 force)
    {
...
public class MUnit : MonoBehaviour
{
    ...
    public AttackType m_base_attackType { get { return m_unit.m_attackType; } }
    public ArmorType m_base_armorType { get { return m_unit.m_armorType; } }
    ...
    public void Damage(MUnit enemy, Damage damage)
    {
        ...
        m_hp -= GetRealDamage(damage, m_unit.m_armorType);
        if (m_hp <= 0) { Dead(); }
        else
        {
            if (damage.m_force != Vector2.zero)
            {
    ...
    public static float[,] m_realDamage = { { 1.0f, 1.0f, 0.5f }, { 1.0f, 0.5f, 0.75f }, { 1.0f, 0.25f, 1.0f } };
    public static int GetRealDamage(Damage damage, ArmorType armorType)
    {
        return (int)(damage.m_power * m_realDamage[(int)damage.m_attackType, (int)armorType]);
    }
    ...
    bool AttackTarget(MUnit unit)
    {
        ...
        if (m_unit.m_attackPrefab != null)
        {
            GameObject obj = (GameObject)Instantiate(m_unit.m_attackPrefab, unit.transform.position, Quaternion.identity);
            obj.GetComponent<FxEffect>().SetEffect(this,unit, new Damage(m_base_attackType, m_attackDamage, (unit.transform.position - transform.position).normalized * m_unit.m_attackForce));
        }
